...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    float VX = (analogRead(X_out) * 5.0) / 1023; //tention de tangage
    float VY = (analogRead(Y_out) * 5.0) / 1023; //tention de roulis
    float VZ = (analogRead(Z_out) * 5.0) / 1023; //tention de lacet
    ...
    int VYi = VY * 100;
    int VZi = VZ * 100;
    //sprintf(payload, "R%dT%dL%dX%dY%dZ%d", Roulis, Tangage, Lacet, VXi, VYi,VZi);
    
    sprintf(payload, "R%dT%dL%d", Roulis, Tangage, Lacet);


    Serial.print(payload);


    // Verification de la disponibilite du channel
