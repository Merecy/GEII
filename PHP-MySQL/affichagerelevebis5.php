...
<body>
    ...
    echo "SELECT idStation, longitude, latitude FROM stations WHERE idStation='".$_POST['choixstation']."';";
    $req=$bdd->query("SELECT idStation, longitude, latitude FROM stations WHERE idStation='".$_POST['choixstation']."';");
    $b=0;
...
<?php
echo "</br>";
$req=$bdd->query("SELECT nom, dateheure, t, ff FROM releves INNER JOIN stations ON stations.idStation=releves.idStation WHERE nom='".$_POST['choixstation']."' AND dateheure=20190304".$_POST['choixDateHeure']." ORDER BY dateheure DESC;");
$f=0;

if($req->fetch()==NULL){
    echo "Il n'y a de données disponible </br>";
}
else{
    $req=$bdd->query("SELECT nom, dateheure, t, ff FROM releves INNER JOIN stations ON stations.idStation=releves.idStation WHERE nom='".$_POST['choixstation']."' AND dateheure=20190304".$_POST['heure']." ORDER BY dateheure DESC;");
